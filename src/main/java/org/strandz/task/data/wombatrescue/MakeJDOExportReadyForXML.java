...
public class MakeJDOExportReadyForXML
{
    private static DataStore dataStore;

    /**
     * Will have set WombatrescueApplicationData.storageType to XML
     * before use this.
     *
     * @param s
     */
    ...
    private static void substitutenullWithNULL(
        DataStore data, Class clazz, Object nullVersion)
    {
        Err.error("Coded before dummy started being used for JDO DBs, so fix first");
        List newList = new ArrayList();
        List shifts = (List) data.get(clazz);
        for(Iterator iter = shifts.iterator(); iter.hasNext();)
        {
            Object whichShift = iter.next();
            if(whichShift == null)
            {
                newList.add(nullVersion);
                Err.pr("Found a null in " + clazz.getName());
            }
            else
            {
                newList.add(whichShift);
            }
        }
        data.set(clazz, newList);
    }

    private static void update(DataStore data)
    {
        substitutenullWithNULL(data, POJOWombatData.WHICH_SHIFT, WhichShift.NULL);
        substitutenullWithNULL(data, POJOWombatData.SENIORITY, Seniority.NULL);
        substitutenullWithNULL(data, POJOWombatData.SEX, Sex.NULL);
    }
}
