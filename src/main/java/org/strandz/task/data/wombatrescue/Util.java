...
public class Util
{
    static void debugWorkerSlots( List rosterSlots, String firstName, String surname)
    {
        ...
        for(Iterator iterator = rosterSlots.iterator(); iterator.hasNext();)
        {
            ...
            if(worker != null)
            {
                String name = worker.getChristianName();
                String secondName = worker.getSurname();
                if(name != null && secondName != null)
                {
                    if(name.equals( firstName) && secondName.equals( surname))
                    {
                        Err.pr( "RS: <" + rosterSlot + ">");
                        count++;
                    }
                }
            }
        ...
        {
            Err.pr( "No matches");
            Print.prList( rosterSlots, "RosterSlots", false);
        }
        else
        {
            Err.pr( "Altogether " + count + " matches");
        }
    }
}
