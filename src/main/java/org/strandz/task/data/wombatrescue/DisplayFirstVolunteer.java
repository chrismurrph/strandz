...
public class DisplayFirstVolunteer
{
    ...
    private static void display(DataStore data)
    {
        ...
        {
            Err.pr(
                "Have a NULL vol will try to display: isDummy() ?: "
                    + nullVol.isDummy());
            Err.pr(nullVol.toString());
            if(!nullVol.isDummy())
            {
                Err.error("Expected the NULL vol to have isDummy() == true");
            }
        }
    }
}
