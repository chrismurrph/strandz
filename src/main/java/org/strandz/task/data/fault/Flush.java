...
public class Flush
{
    ...
    private static void processParams(String s[])
    {
        if(s[0].equals("FaultData"))
        {
            ...
            if(!REFILL_LOVS_ONLY)
            {
                ...
                {
                    Err.error(
                        "Cannot flush the " + FaultApplicationData.PROD + " database");
                }
            }

            DataStore td = null;
            String txt = "Flushed FaultData using " + s[1];
            if(s.length == 2)
            {
                td = FaultApplicationData.getNewInstance(s[1]).getData();
            }
            else
            {
                td = FaultApplicationData.getNewInstance(s[1], s[2]).getData();
                txt += ", " + s[2];
            }
            if(!REFILL_LOVS_ONLY)
            {
    ...
    private static void fillLOVs(DataStore data)
    {
        ...
        // work for XML
        data.update(FaultData.FAULT_STATUS,
            new ArrayList(Arrays.asList(FaultStatus.OPEN_VALUES)));
        data.update(FaultData.PRODUCT,
            new ArrayList(Arrays.asList(Product.OPEN_VALUES)));
        data.commitTx();
    }
}
