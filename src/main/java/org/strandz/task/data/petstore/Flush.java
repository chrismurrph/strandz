...
public class Flush
{
    ...
    private static void processParams(String s[])
    {
        if(s[0].equals("PetStoreData"))
        {
            ...
            if(!REFILL_LOVS_ONLY)
            {
                ...
                {
                    Err.error(
                        "Cannot flush the " + PetstoreApplicationData.PROD + " database");
                }
            }

            DataStore td = null;
            String txt = "Flushed PetStoreData using " + s[1];
            if(s.length == 2)
            {
                td = PetstoreApplicationData.getNewInstance(s[1]).getData();
            }
            else
            {
                td = PetstoreApplicationData.getNewInstance(s[1], s[2]).getData();
                txt += ", " + s[2];
            }
            if(!REFILL_LOVS_ONLY)
            {
    ...
    private static void fillLOVs(DataStore data)
    {
        ...
        // work for XML
        data.update(PetstoreData.ACCOUNT_STATUS,
            new ArrayList(Arrays.asList(AccountStatus.OPEN_VALUES)));
        data.update(PetstoreData.CATEGORY,
            new ArrayList(Arrays.asList(Category.OPEN_VALUES)));
        data.update(PetstoreData.ITEM_STATUS,
            new ArrayList(Arrays.asList(ItemStatus.OPEN_VALUES)));
        data.update(PetstoreData.SUPPLIER_STATUS,
            new ArrayList(Arrays.asList(SupplierStatus.OPEN_VALUES)));
        data.commitTx();
    }
}
