...
public class Flush
{
    ...
    private static void processParams(String s[])
    {
        if(s[0].equals("TimesheetData"))
        {
            ...
            if(!REFILL_LOVS_ONLY)
            {
                ...
                {
                    Err.error(
                        "Cannot flush the " + TimesheetApplicationData.PROD + " database");
                }
            }

            DataStore td = null;
            String txt = "Flushed TimesheetData using " + s[1];
            if(s.length == 2)
            {
                td = TimesheetApplicationData.getInstance(s[1]).getData();
                // Err.error( "Must specify a storage type to flush Timesheet Data");
            }
            else
            {
                td = TimesheetApplicationData.getInstance(s[1], s[2]).getData();
                txt += ", " + s[2];
            }
            if(!REFILL_LOVS_ONLY)
            {
    ...
    {
        // Err.error( "Timesheet fixed b4 call again");
        data.rollbackTx();
        data.startTx();
        // For XML needed to use new ArrayList() as result of Arrays.asList() did not
        // work for XML
        // DO NOT HAVE ANY LOVs, BUT WHEN DO, FOR EACH THIS LINE:
        // data.update( TimesheetData.SENDING_MEDIUM, new ArrayList( Arrays.asList( SendingMedium.VALUES)));
        data.commitTx();
    }
}
