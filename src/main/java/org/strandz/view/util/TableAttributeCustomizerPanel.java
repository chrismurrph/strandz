...
public class TableAttributeCustomizerPanel extends JPanel
{
    ...
    private final Label lColumnHeading = new Label("Column Heading: ");
    public TextField tfColumnHeading = null;
    private final JPanel jpColumnHeadingRow = new JPanel();
    private final Label lType = new Label("Type:  ");
    public LabelField tfType = null;
    private final JPanel jpTypeRow = new JPanel();
    private final Label lDOField = new org.strandz.lgpl.widgets.Label("DO Field:  ");
    public LabelField lfDOField = null;
    private final JPanel jpDOFieldRow = new JPanel();
    private final Label lOrdinal = new Label("Ordinal: ");
    public TextField tfOrdinal = null;
    private final JPanel jpOrdinalRow = new JPanel();
    public JCheckBox cbEnabled = new JCheckBox();
    private final JPanel jpEnabledRow = new JPanel();

    public TableAttributeCustomizerPanel()
    {
    ...
    public void init()
    {
        ...
        add(jpNameRow);
        dim = new Dimension((int) lColumnHeading.getPreferredSize().getWidth(),
            SHORT_ROW_HEIGHT);
        tfColumnHeading = new TextField();
        tfColumnHeading.setHeight(SHORT_ROW_HEIGHT);
        tfColumnHeading.setName("tfColumnHeading");
        lColumnHeading.setPreferredSize(dim);
        lColumnHeading.setMaximumSize(dim);
        lColumnHeading.setVerticalAlignment(SwingConstants.CENTER);
        jpColumnHeadingRow.setLayout(
            new BoxLayout(jpColumnHeadingRow, BoxLayout.X_AXIS));
        jpColumnHeadingRow.setAlignmentX(SwingConstants.LEFT);
        jpColumnHeadingRow.add(lColumnHeading);
        jpColumnHeadingRow.add(tfColumnHeading);
        add(jpColumnHeadingRow);
        ...
        add(jpDOFieldRow);
        dim = new Dimension((int) lOrdinal.getPreferredSize().getWidth(),
            SHORT_ROW_HEIGHT);
        tfOrdinal = new TextField();
        tfOrdinal.setHeight(SHORT_ROW_HEIGHT);
        tfOrdinal.setName("tfOrdinal");
        lOrdinal.setPreferredSize(dim);
        lOrdinal.setMaximumSize(dim);
        lOrdinal.setVerticalAlignment(SwingConstants.CENTER);
        jpOrdinalRow.setLayout(new BoxLayout(jpOrdinalRow, BoxLayout.X_AXIS));
        jpOrdinalRow.setAlignmentX(SwingConstants.LEFT);
        jpOrdinalRow.add(lOrdinal);
        jpOrdinalRow.add(tfOrdinal);
        add(jpOrdinalRow);
