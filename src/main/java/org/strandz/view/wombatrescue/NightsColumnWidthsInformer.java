...
public class NightsColumnWidthsInformer implements ColumnWidthsInformerI
{
    static int CHAR_WIDTH = Utils.UNSET_INT;
    static final int SOME_EXTRA_PADDING = 10;
    static final int MORE_EXTRA_PADDING = 20;
    ...
    public double getColumnWidthAt( int column, int numOfColumns)
    {
        ...
        if(column == 0)
        {
            result = 12 + SOME_EXTRA_PADDING;
        }
        else if(column == 1)
        {
            result = 55 + SOME_EXTRA_PADDING; //50 is touching
        }
        else if(column == 6) //To accomodate explanation text that goes into last cell
        {
            result = Night.NAME_WIDTH * CHAR_WIDTH + MORE_EXTRA_PADDING;
        }
        else
        {
