...
import org.strandz.applic.util.OnceLoggedInAction;
...
abstract public class AbstractWRTask
{
    private class LocalOnceLoggedInAction extends OnceLoggedInAction
    {
        public void update(EntityManagedDataStore dataStore)
        {
            AbstractWRTask.this.update( dataStore);    
        }
    }
    ...
    public void processParams(WombatConnectionEnum connectionEnum, String dialogName)
    {
        DataStoreFactory dataStoreFactory = new WombatDataStoreFactory();
        dataStoreFactory.addConnection( connectionEnum);
        String user = getUsername();
        String pass = getPassword();
        if(Utils.bothNull( user, pass))
        {
            ...
            user = (String)details[0];
            pass = (String)details[1];
        }
        LoginHelper.Params params = LoginHelper.newParams( 
                connectionEnum, 
                dataStoreFactory,
                dialogName,
                new LocalOnceLoggedInAction(),
                user, 
                pass
                );  
    ...
    abstract public void update(EntityManagedDataStore dataStore);
}
