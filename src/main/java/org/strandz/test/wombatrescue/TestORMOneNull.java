...
public class TestORMOneNull extends TestCase
{
    private EntityManagedDataStore dataStore;

    public static void main(String s[])
    {
        TestORMOneNull obj = new TestORMOneNull();
        obj.setUp();
        obj.testOneNullWorker();
        System.exit(0);
    }

    public TestORMOneNull()
    {
        if(dataStore == null)
        {
    ...
    public void testOneNullWorker()
    {
        dataStore.startTx();
        DomainQueriesI queriesI = dataStore.getDomainQueries();
        List vols = queriesI.executeRetList(WombatDomainQueryEnum.NULL_WORKER);
        assertTrue( vols.size() == 1);
        dataStore.commitTx();
    }
}