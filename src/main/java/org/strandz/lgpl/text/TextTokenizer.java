/**
 * Got from http://www.cs.arizona.edu/~reges/teachers/TextReader.java
 * Adapted to work with StringBuffer only
 */
...
public class TextTokenizer implements WordTokenizer
{
    private StringBufferReader myReader;       
    ...
    private static class StringBufferReader
    {
     ...
     */
    public TextTokenizer( StringBuffer input)
    {
        myReader = new StringBufferReader( input);
        myUndefinedState = true;
    }

    /**
     * @return where the reader will read from when it next does a read is
     * myReader.at - however we return the position of the last character 
     * read
     */
    public int position()
    {
        int result;
        if(!myReader.finished)
        {
            result = myReader.at-1;
        }
        else
        {
            result = myReader.at;
    ...
    {
        return myReader.finished;
    }

    private int read()
    {
        int result = myReader.read();
        return result;
    }   
