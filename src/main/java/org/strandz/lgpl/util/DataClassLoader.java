 ...
 * @author Chris Murphy
 */
public class DataClassLoader extends SpecialClassLoader
{
        ...
        boolean debugging)
    {
        ...
        id = ++constructedTimes;
        pr("##CONSTRUCTED DataClassLoader with path " + basePath + " ID " + id);
        pr("Special files are [ID " + id + "]:");
        if(!dynamicDirectoryFiles.isEmpty())
        {
            pr(dynamicDirectoryFiles.get(0).toString());
        }
        if(id == 0)
        {
            Err.stack();
    ...
    {
        String result = null;
        result = "DataClassLoader with path " + basePath + " ID " + id;
        return result;
    }
}
