...
import org.strandz.view.util.ReferenceLinkAttributeCustomizerPanel;

import java.util.ArrayList;
import java.util.List;

/**
 * This class is used for editing a ReferenceLinkAttribute at DT.
 *
 * @author Chris Murphy
 */
public class ReferenceLinkAttributeCustomizer extends OneRowSdzBag
        implements CustomizerI
{
    private Node node;
    private Cell cell;
    private ArrayList beans = null;
    private ReferenceLinkAttributeCustomizerPanel panel;
    private ReferenceLinkAttribute object;

    public ReferenceLinkAttributeCustomizer()
    {
        setName( NameUtils.tailOfPackage( this.getClass().getName()));
        MessageDlg.setDlgParent( this);
        panel = new ReferenceLinkAttributeCustomizerPanel();
        panel.init();
        cell = new Cell();
        cell.setClazz(new Clazz(org.strandz.core.interf.ReferenceLinkAttribute.class));
        ...
        setName("ReferenceLinkAttributeCustomizer");
    }

    public void setObject(Object obj)
    {
        beans = new ArrayList();
        object = (ReferenceLinkAttribute) obj;
        beans.add(object);
    }
    ...
    class LocalDataFlowListener implements DataFlowTrigger
    {
        public void dataFlowPerformed(DataFlowEvent evt)
        {
            ...
            if(evt.getID() == DataFlowEvent.PRE_QUERY)
            {
                ...
                // Could have had RO Attribute rather than this
                if(object.getIndependent() != null)
                {
                    panel.tfMasterNode.setText(
                        object.getIndependent().getMasterNode().getName());
                }
                else
                {
                    Print.pr("DON'T HAVE INDEP YET");
                }
            }
        }
    }
