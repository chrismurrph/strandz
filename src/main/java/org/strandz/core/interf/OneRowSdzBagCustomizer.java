...
import org.strandz.view.util.OneRowSdzBagCustomizerPanel;

import java.util.ArrayList;
import java.util.List;

/**
 * This class is used for editing a OneRowSdzBag at DT.
 *
 * @author Chris Murphy
 */
public class OneRowSdzBagCustomizer extends OneRowSdzBag
    implements CustomizerI
{
    private Node node;
    private Cell cell;
    private ArrayList beans = null;
    private OneRowSdzBagCustomizerPanel panel;
    private OneRowSdzBag sc;

    public OneRowSdzBagCustomizer()
    {
        setName( NameUtils.tailOfPackage( this.getClass().getName()));
        MessageDlg.setDlgParent( this);
        panel = new OneRowSdzBagCustomizerPanel();
        panel.init();
        cell = new Cell();
        cell.setClazz(new Clazz(org.strandz.core.interf.OneRowSdzBag.class));

        FieldAttribute fa = new FieldAttribute("name", panel.tfName);
        cell.addAttribute(fa);
        fa = new FieldAttribute("pane", panel.tfView);
        cell.addAttribute(fa);
        cell.setName("cell");
        node = new Node();
        ...
        setName("OneRowStrandControlCustomizer");
    }
