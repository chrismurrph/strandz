...
/**
 * This is a class for executing a RosterServiceI on both the developer workstation
 * against a locally running tomcat and the production server: as DEMO_WOMBAT_KODO_SERVICE is 
 * used as the production service, then this is all there is ie. this is what Spring on the 
 * server calls
 */
public class ServerRosterService extends AbstractRosterService
{
    public ServerRosterService()
    {
    ...
    {
        ServerRosterService service = new ServerRosterService();
        Err.pr( "Created " + service);
        Err.pr( "About to try and call display service:");
        MonthTransferObj month = new MonthTransferObj( MonthInYear.OCTOBER, 2008);
    ...
    {
        return "ServerRosterService with dataStore " + getDataStore();
    }
