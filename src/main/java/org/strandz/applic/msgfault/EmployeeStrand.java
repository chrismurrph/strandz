...
public class EmployeeStrand extends VisibleStrand
{
    private static String filename = "C:\\cml\\dt\\msgfault\\EmployeeMaintenance.xml";
    public MsgfaultQueries queries;
    //public SdzBagI sbI;
    public EmployeeTriggers triggers;
    private EmployeeDT dt;

    public EmployeeStrand()
    {
        init();
    }

    public EmployeeStrand(Application a)
    {
        super(a);
        init();
    }
    ...
    private void init()
    {
        setDataStore(MsgfaultApplicationData.getInstance().getData());
        queries = MsgfaultApplicationData.getInstance().getQueries();
    }
    ...
    public void sdzSetup()
    {
        ...
        dt = new EmployeeDT(sbI);
        triggers = new EmployeeTriggers(getDataStore(), queries, dt, sbI);
        if(getApplication() != null)
        {
            initSdzBag((SdzBag) sbI);
            setPanelNodeTitle(dt.ui0, dt.employeeNode, "employeeNode");
        }
        else
        {
