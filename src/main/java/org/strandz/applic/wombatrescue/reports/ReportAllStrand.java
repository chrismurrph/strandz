...
public class ReportAllStrand extends VisibleStrandAdapter implements VisibleStrandI
{
    ...
    public ReportAllStrand(Application a)
    {
        this.application = a;
    }
    ...
    public void display(boolean b)
    {
        if(b)
        {
            ...
            List<Reports.AllInfo> allInfo = reports.reportAll();
            textAreaDisplayPanel.getTextArea().setText("");
            textAreaDisplayPanel.getTextArea().append( Reports.AllInfo.csvFormat( allInfo));
            surface.add(textAreaDisplayPanel, BorderLayout.CENTER);
            application.endActivity( this.getClass().getName());
        }
    }

    private String simpleFormat( List<Reports.AllInfo> allInfo)
    {
