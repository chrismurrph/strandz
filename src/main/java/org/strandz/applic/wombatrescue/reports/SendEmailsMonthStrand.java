...
public class SendEmailsMonthStrand extends VisibleStrandAdapter implements VisibleStrandI
{
    private Application application;
    private Reports reports;

    public SendEmailsMonthStrand(Application a)
    {
        this.application = a;
    }
    ...
    public void display(boolean b)
    {
        if(b)
        {
            ...
            else
            {
                ...
                //Err.pr( WombatNote.firstLastDaysEmails, "first day: " + NewMonth.chkFirstLastDaysEmails( first));
                //Err.pr( WombatNote.firstLastDaysEmails, "last day: " + NewMonth.chkFirstLastDaysEmails( last));

                String periodTxt = "the month of "
                    + RosterSessionUtils.getGlobalCurrentMonth().month;
                String msgTitle = RosterSessionUtils.getProperty( "organisationAcroymn") + " Roster for "
                    + RosterSessionUtils.getGlobalCurrentMonth().month;
                
                ParticularRosterI bo = RosterSessionUtils.getGlobalCurrentParticularRoster();
                MessagesContainer con = bo.writeAllRosteringMsgs(first, last, periodTxt);
                Informer emailer = new Emailer();
                emailer.setMsgs(msgTitle, con.emails);
                Err.pr( WombatNote.MISSING_EMAILS, "Num of volunteer emails going to send is " + con.emails.size());
                emailer.sendMsgs();
                
                msgTitle = "Buddy info for "
                    + RosterSessionUtils.getGlobalCurrentMonth().month;
                con = reports.writeAllBuddyMsgs(first, last);
                emailer.setMsgs(msgTitle, con.emails);
                Err.pr( WombatNote.MISSING_EMAILS, "Num of BM emails going to send is " + con.emails.size());
                emailer.sendMsgs();
