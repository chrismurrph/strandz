...
import org.strandz.lgpl.util.GlobalEventQueue;

public class StartupLinodeProd extends Startup
{
    private static final GlobalEventQueue globalEventQueue = new GlobalEventQueue();

    public StartupLinodeProd()
    {
        Err.pr("In StartupLinodeProd");
        init();
        dataStoreFactory.addConnection(WombatConnectionEnum.LINODE_PROD_TERESA);
        RosterSessionUtils.setDataStore(dataStoreFactory.getDataStore());
    }

    public static void main(String[] args)
    {
        new StartupLinodeProd().setVisible(true);
    }
}
