...
public class MaintainOrdersTriggers
{
    public DataStore data;
    public PetstoreQueries queries;
    public MaintainOrdersDT dt;

    public MaintainOrdersTriggers(DataStore data,
    ...
    )
    {
        ...
        dt.orderReferenceDetailNode.addDataFlowTrigger(new DataFlowT1());
        dt.lineitemReferenceDetailNode.addDataFlowTrigger(new DataFlowT2());
    ...
    static class HandlerT implements ValidationHandlerTrigger
    {
        public void handleError(ApplicationError e)
        {
            ...
            {
                Print.prThrowable(e, "MaintainOrdersTriggers.HandlerT");
            }
        }
    }
    ...
    class DataFlowT1 implements DataFlowTrigger
    {
        public void dataFlowPerformed(DataFlowEvent evt)
        {
            if(evt.getID() == DataFlowEvent.PRE_QUERY)
            {
                ...
                dt.orderReferenceDetailCell.setData(list);
            }
        }
    }
    ...
    class DataFlowT2 implements DataFlowTrigger
    {
        public void dataFlowPerformed(DataFlowEvent evt)
        {
            if(evt.getID() == DataFlowEvent.PRE_QUERY)
            {
                List list = data.query(org.strandz.data.petstore.objects.LineItem.class);
                dt.lineitemReferenceDetailCell.setData(list);
            }
        }
    }
