...
public class MaintainOrdersStrand extends VisibleStrand
{
    private static String filename = "C:\\cml\\dt\\petstore\\PetStore.xml";
    public PetstoreQueries queries;
    //public SdzBagI sbI;
    public MaintainOrdersTriggers triggers;
    private MaintainOrdersDT dt;

    public MaintainOrdersStrand()
    {
        init();
    }

    public MaintainOrdersStrand(Application a)
    {
        super(a);
        init();
    }
    ...
    {
        MaintainOrdersStrand obj;

        RunLater(MaintainOrdersStrand obj)
        {
            this.obj = obj;
        }
    ...
    public void sdzSetup()
    {
        ...
        dt = new MaintainOrdersDT(sbI);
        triggers = new MaintainOrdersTriggers(getDataStore(), queries, dt, sbI);
        if(getApplication() != null)
        {
            initSdzBag((SdzBag) sbI);
            setPanelNodeTitle(dt.ui0, dt.accountNode, "accountNode");
            // setPanelNodeTitle( dt.ui1, dt.orderReferenceDetailNode, "orderReferenceDetailNode");
            // setPanelNodeTitle( dt.ui2, dt.lineitemReferenceDetailNode, "lineitemReferenceDetailNode");
        }
        else
        {
            Err.error(
