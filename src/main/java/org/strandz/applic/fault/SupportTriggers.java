...
public class SupportTriggers
{
    ...
    public SupportTriggers(DataStore data,
    ...
    )
    {
        ...
        this.dt = dt;
        dt.supportNode.addDataFlowTrigger(new DataFlowT0());
    ...
    static class HandlerT implements ValidationHandlerTrigger
    {
        public void handleError(ApplicationError e)
        {
            ...
            {
                Print.prThrowable(e, "SupportTriggers.HandlerT");
            }
        }
    }
    ...
    class DataFlowT0 implements DataFlowTrigger
    {
        public void dataFlowPerformed(DataFlowEvent evt)
        {
            if(evt.getID() == DataFlowEvent.PRE_QUERY)
            {
                ...
                Collection c = queries.queryAllSupportCalls();
                List enterQryAttribs = dt.supportNode.getEnterQueryAttributes();
                c = InterfUtils.refineFromMatchingValues(c, enterQryAttribs);
                //Was a NOP
                //data.setRefinedList( org.strandz.data.fault.objects.SupportCall.class, c);
                dt.supportCell.setData(c);
            }
        }
    }
    ...
    {
        dt.faultStatusLookupCell.setLOV((List) data.get(FaultData.FAULT_STATUS));
        dt.productLookupCell.setLOV((List) data.get(FaultData.PRODUCT));

        // Rather than this
        // dt.clientContactLookupCell.setLOV( (List)data.get( FaultData.CLIENT_CONTACT));
        // do this
        Collection contacts = queries.queryAllClientContacts();
        List pickContacts = new ArrayList(contacts);
        pickContacts.add(0, ClientContact.NULL);
        dt.clientContactLookupCell.setLOV(pickContacts);
    }
}
